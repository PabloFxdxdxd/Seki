...
class AppDatabase extends _$AppDatabase {
  ...
  Future<int> totalUsuarios() async {
    ...
    final consulta = selectOnly(user)
      ..addColumns([usuarios])
      ..where(
        user.type.equals("user"),
      ); //hace la consulta y sólo ve la tabla user
  ...
  Future<double> cumplimientoGlobal() async {
    ...
    final consulta = selectOnly(activity)..addColumns([registros]);
    ...
    final cumplidos = countAll(
      filter: activity.isActive.equals(
        false,
      ), //True sin terminar, False cumplida
    ); //contará solo los registros que cumplan el filtro
    final consultaCumplidos = selectOnly(activity)..addColumns([cumplidos]);
    final totalCumplidos =
        (await consultaCumplidos.getSingle()).read(cumplidos) ?? 0;
  ...
  Future<int> registrosHoy() async {
    ...
    final consulta = selectOnly(activity)
      ...
      ..where(
        activity.dueDate.isBetweenValues(inicioDia, finDia),
      ); //registros del día
  ...
  Future<List<MonthlyGrowth>> usuariosPorMes() async {
    ...
    for (int i = 5; i >= 0; i--) {
      ...
      final consulta = selectOnly(user)
        ...
        ..where(
          user.type.equals("user") &
              user.createdAt.isBetweenValues(fecha, sigMes),
        ); //usuarios del mes
      final total = await consulta.getSingle();
      result.add(
        MonthlyGrowth(
  ...
  Future<List<TopUser>> getTopUsers({int limit = 3}) async {
    final filas = await (select(user).join([
      leftOuterJoin(
        activity,
        activity.userId.equalsExp(user.id) & activity.isActive.equals(false),
      ),
    ])).get();

    final Map<int, _UserLogAccum> accum = {};
    for (final fila in filas) {
      if (fila.readTable(user).type != "user") {
        continue; //ignora admins
      }
    ...
    final sorted =
        accum.values
            .map((a) => TopUser(nom: a.name, totalReg: a.total, completado: 0))
            .toList()
