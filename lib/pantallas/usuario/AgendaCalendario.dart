...
class _AgendaCalendarioState extends State<AgendaCalendario> {
  ...
  bool _esFechaPasada(DateTime fecha) {
    ...
    return fechaSeleccionada.isBefore(
      fechaHoy,
    ); //Retorna true si es antes de hoy
  }

  //LOGICA PARA REPETIR HÁBITOS Y MARCAR FECHAS
  Map<DateTime, List<ActivityData>> _agruparEventos(
    ...
    for (var act in actividades) {
      if (act.type == 'Tarea' || act.type == 'Task') {
        ...
        DateTime fin =
            act.dueDate ?? DateTime.now().add(const Duration(days: 365));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
                      ...
                      return Positioned(
                        ...
                        child: Row(
                          ...
                          children: events.take(4).map((event) {
                            bool esHabito =
                                (event as ActivityData).type == 'Hábito' ||
                                event.type == 'Habit';
                            return Container(
                              ...
                              decoration: BoxDecoration(
                                color: esHabito
                                    ? const Color.fromARGB(255, 247, 172, 149)
                                    : const Color.fromARGB(255, 244, 183, 243),
      ...
      floatingActionButton:
          _selectedDay != null && _esFechaPasada(_selectedDay!)
                ...
                child: FloatingActionButton(
                  onPressed: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => const FormActividad(),
                      ),
                    );
                  },
